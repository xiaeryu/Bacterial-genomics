...
if(!defined($ARGV[3])){
        die "Usage:<query file for the blast search><input _blast.out file><coverage,0 to 1><identity, 0 to 100>\n";
}
...
print "query\thit\tidentity\talignment_length\tmismatches\tgap_opens\tq.start\tq.end\ts.start\ts.end\te-value\tbit_score\n";
...
while(<INPUT>){
        ...
        if(!/^#/){
                my @tmp=split(/\s+/);
                if(($tmp[2]>=$identity) && ($tmp[3]>=$coverage*$storage{$tmp[0]})){
                        print "$_\n";
                }
        }
}
